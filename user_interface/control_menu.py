...
class ControlMenu:
    def __init__(
        ...
        max_label_width, max_btn_width, max_row_height = self.build_control_displays()
        self.row_height = max_row_height
        self.position_control_displays(
            y_pos, row_gap, col_gap, max_label_width, max_btn_width, max_row_height
        )
        col_height = (max_row_height * num_rows) + (row_gap * (num_rows - 1))
    ...
    def build_control_displays(self):
        ...
        return max_label_width, max_btn_width, max_row_height



    def position_control_displays(self, starting_y, row_gap, col_gap, label_width, btn_width, row_height):
        canvas_width, _ = self.canvas_size
        y_pos = starting_y
        label_x_pos = (canvas_width//2) - label_width - (col_gap//2)
        btn_x_pos = (canvas_width//2) + (col_gap//2)
        for control_display in self.control_displays:
            label, button = control_display
            label.set_topleft((label_x_pos, y_pos))
            button.set_center((btn_x_pos + btn_width//2, y_pos + row_height//2))

            y_pos += row_height + row_gap
    ...
    def create_editing_popup(self):
        self.input_msg = TextDisplay(
            topleft=(0, 0), font=self.font, color=self.color, text="ENTER AN INPUT"
        )
        ...
        pygame.draw.rect(self.popup_background, self.color, border_rect, width=border_width)
